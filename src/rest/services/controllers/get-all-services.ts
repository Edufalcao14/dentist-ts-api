...
export const initGetAllServicesController = (usecases: Usecases) => {
  return async (req: Request & { context: Context }, res: Response<ServiceDto[] | { message: string }>) => {
    try {
      const items = await usecases.service.getAll(req.context);
