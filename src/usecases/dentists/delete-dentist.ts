...
export const deleteDentist = async (context: Context, id: bigint): Promise<void> => {
  if (!context.auth.isAuthenticated || !context.auth.tenantResolved) {
    throw new UnauthorizedError('Authentication required');
  }

  const existing = await context.repositories.dentists.getById(id, context.auth.tenantId);
  if (!existing) throw new NotFoundError('Dentist not found');

  await context.repositories.dentists.delete(id);
};
